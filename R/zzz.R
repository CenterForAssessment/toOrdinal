...
`.onAttach` <- function(libname, pkgname) {
    if (interactive()) {
        ...
        installed.version <- utils::packageDescription("toOrdinal")[['Version']]
        cran.version <- tryCatch(
            green("v", pkgsearch::cran_package("toOrdinal")[['Version']], sep=""),
            error = function(e) red("Not Available"),
            warning = function(w) red("Not Available"))
        dev.version <- get_dev_version("toOrdinal")

        # Define a friendly startup message
		message_text <- paste0(
		    magenta(bold("\uD83C\uDF89 toOrdinal v", installed.version, sep="")), " - ", toOrdinal::toOrdinalDate("2024-12-4"), "\n",
			strrep("\u2501", 40), "\n",
    	    bold("\U1F4E6 CRAN: "), cran.version, "\n",
    	    bold("\U1F527 Dev: "), dev.version, "\n",
			strrep("\u2501", 40), "\n",
		    "\U1F4A1 Tip: ", magenta(bold("> help(package=\"toOrdinal\")")), "\n",
